...
Multi select input chips are created when a separator is used (typed or pasted) or when values are pasted. Typed text without a separator is treated as pure search text and never selects an option automatically.
