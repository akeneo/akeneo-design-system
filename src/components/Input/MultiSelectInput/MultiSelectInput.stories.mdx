...
<Meta
  ...
  argTypes={{
    ...
    keepDropdownOnSelect: {
      control: {type: 'boolean'},
      description: 'Keeps the dropdown open after an option is selected.',
      table: {type: {summary: 'boolean'}, defaultValue: {summary: 'false'}},
    },
    resolvePastedLabels: {
      control: {type: 'boolean'},
      description: 'Resolves pasted option labels to their values.',
      table: {type: {summary: 'boolean'}, defaultValue: {summary: 'false'}},
    },
    separators: {
      control: {type: 'object'},
      description: 'List of separators used to create chips.',
      table: {type: {summary: 'string[]'}, defaultValue: {summary: "['\\\\r', '\\\\n', '\\\\t', ',', ';']"}},
    },
  }}
/>
...
Multi select input chips are created when a separator is used (typed or pasted) or when values are pasted. Typed text without a separator is treated as pure search text and never selects an option automatically. When `resolvePastedLabels` is enabled, pasted option labels are resolved to their values.
...
The separator list can be customized with the `separators` property. For example, `['\\s', ',', ';']` also treats spaces as separators. Do not enable whitespace separation when valid option values or labels can contain spaces.
...
<Canvas>
  <Story name="With Locales">
    {args => {
      const [value, setValue] = useState([]);
      return (
        <SpaceContainer height={170}>
          <MultiSelectInput {...args} removeLabel="Remove" value={value} onChange={setValue}>
            <MultiSelectInput.Option value="en_US" enableLocaleRender={true}>English (United States)</MultiSelectInput.Option>
            <MultiSelectInput.Option value="fr_FR" enableLocaleRender={true}>French (France)</MultiSelectInput.Option>
            <MultiSelectInput.Option value="de_DE" enableLocaleRender={true}>German (Germany)</MultiSelectInput.Option>
          </MultiSelectInput>
        </SpaceContainer>
      );
    }}
...
By default, the MultiSelectInput automatically selects options matching the text when it is pasted (e.g., pasting "en_US" or "en_US;fr_FR" selects those options) or when it contains a separator (e.g., typing "en_US,"). Typed text without a separator is pure search text and never selects an option automatically. The remaining automatic selection can be disabled using the `disableAutoSelect` property.

When `disableAutoSelect` is set to `true`:
- Pasted text is treated as pure search text and is not automatically matched against option values
