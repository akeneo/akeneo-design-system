      ...
      return (
        <CardGrid>
          ...
          <Card {...args} src={getStaticImage('akenec_200.jpg')}>
            <Card.BadgeContainer>
              <Badge level="tertiary">tag</Badge>
...
Use `dimmed` to visually indicate that the item represented by the card is inactive or unavailable,
without affecting card interactivity. Only the image loses opacity — clicking, selecting, and links continue to work normally.

This differs from `disabled`, which blocks all interaction entirely.
...
<Canvas>
  <Story name="Dimmed">
    {args => {
      ...
      return (
        <CardGrid>
          <Card {...args} src={getStaticImage('akenea_200.jpg')} onSelect={() => setFirstSelected(!firstSelected)} isSelected={firstSelected}>
            Active card (normal)
          </Card>
          <Card {...args} dimmed src={getStaticImage('akeneb_200.jpg')} onSelect={() => setSecondSelected(!secondSelected)} isSelected={secondSelected}>
            Dimmed card (inactive item)
          </Card>
        </CardGrid>
      );
    }}
